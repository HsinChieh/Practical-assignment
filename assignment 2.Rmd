...
From the result of Q5, there are three variables (main_dish, main_prep, n). I did 
...
To count the most repeated values in many columns, we need to use `start_with()` & `gather()`.
The former is to select the same few words in different columns, while the latter is to gather a set of column name in a type column and their values in another column. Besides,`str_remove()` function used to delete some characters in a string since I want to make them same type. (\d: numeric 0-9; \d+: more than one numeric, such as 11,12...)
